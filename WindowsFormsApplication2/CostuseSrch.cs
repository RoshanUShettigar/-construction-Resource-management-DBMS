...
{
    public partial class CostuseSrch : UserControl
    {
        public CostuseSrch()
        {
            InitializeComponent();
        ...
        private void btnscrh_Click(object sender, EventArgs e)
        {
            ...
            con.Open();
            SqlCommand query = new SqlCommand("select * from Cost where( pj_id='" + srchtextBox1.Text + "')", con);
            string str = "select * from Cost where( pj_id='" + srchtextBox1.Text + "')";
            SqlDataAdapter adpt = new SqlDataAdapter(str, con);
            DataTable dt = new DataTable();
            adpt.Fill(dt);
            dataGridViewCostSRCH.DataSource = dt;
